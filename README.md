...
## Прокси
По умолчанию клиент игнорирует системные переменные прокси (если они выставлены некорректно).
Чтобы использовать системный прокси, задайте:
```powershell
...
- Получение API‑ключа: `docs/GROQ_API_KEY.md`
...
cargo test
```
Интеграционный тест Groq (требует ключ и доступ к сети):
```powershell
$env:GROQ_API_KEY="your_key"
cargo test -- --ignored groq_transcribe_smoke
```

Проверка Groq на WAV-файле:
```powershell
$env:GROQ_API_KEY="your_key"
cargo run --bin transcribe_wav -- path\to\audio.wav
```
...
## Release-сборка (NSIS + MSI)
Скрипт собирает релиз, гоняет тесты и делает инсталляторы.
```powershell
.\installer\release.ps1
```
Опции:
- `-SkipTests` — не запускать тесты.
- `-SkipNsis` — не собирать NSIS.
- `-SkipMsi` — не собирать MSI.
